...
const connectors: Connector[] = [
  {
    ...
    name: "Facebook Ads",
    description:
      "Connect Meta account and operate campaigns, ad sets, and creatives from Operon AI.",
  ...
  },
  {
    ...
    name: "Instagram",
    description:
      "Use the same Meta connection for Instagram placements and audience targeting.",
  ...
  },
  {
    ...
    name: "LinkedIn",
    description:
      "Plan thought-leadership, demand-gen campaigns, and audience expansion.",
  ...
  },
  {
    ...
    name: "TikTok Ads",
    description:
      "Launch short-form campaign variants and automate testing loops.",
    coverage: "Planned connector",
  },
  {
    id: "youtube",
    name: "YouTube",
    description:
      "Build video campaign structures and creative adaptation workflows.",
    coverage: "Planned connector",
  },
  {
    id: "pinterest",
  ...
  },
  {
    ...
    name: "Reddit",
    description:
      "Activate community-native ad units and feedback-aware iteration.",
    coverage: "Planned connector",
  },
];
...
export default function SocialPage() {
  ...
  const priorityConnectors = useMemo(
    () =>
      connectors.filter(
        (item) => item.id === "facebook" || item.id === "instagram",
      ),
    [],
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await operonFetch("/meta/status");
  ...
  const connectedCount = useMemo(() => {
    ...
    return connectors.filter(
      (connector) => connector.id === "facebook" || connector.dependsOnMeta,
    ).length;
  }, [status.connected]);
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Connect channels, unlock AI media buying workflows, and operate
              ads from one control surface.
            </p>
          </div>
        ...
        <div className="mx-auto max-w-7xl space-y-6">
          <div className="grid min-h-screen gap-5 lg:grid-cols-2">
            {priorityConnectors.map((connector) => {
              const isConnected =
                status.connected &&
                (connector.id === "facebook" || connector.dependsOnMeta);
                    ...
                    {isConnected ? (
                      ...
                      <Button asChild size="lg" className="w-full rounded-xl">
                        <Link href={connector.href!}>
                          {connector.id === "facebook"
                            ? "Open Facebook Connect Page"
                            : "Open Instagram Connect Page"}
          ...
          {status.connected && !loading ? (
            <div className="grid min-h-screen gap-4 lg:grid-cols-2">
              <div className="flex min-h-168 flex-col rounded-2xl border border-border/70 bg-card/60 p-6">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Facebook and Instagram are now available to your AI operator.
                  Operon can monitor spend, inspect campaign results, and
                ...
                <div className="mt-6 grid gap-3 text-sm sm:grid-cols-2">
                  ...
                  <div className="rounded-xl border border-border/70 bg-background/90 p-4">
                    ...
                    <div className="mt-1 font-medium text-foreground">
                      {status.adAccountsCount ?? 0} detected
                    </div>
                  </div>
                  <div className="rounded-xl border border-border/70 bg-background/90 p-4 sm:col-span-2">
                    ...
                    <div className="mt-1 text-foreground">
                      Campaign triage, budget recommendation, audience
                      experimentation, and creative briefing.
                    </div>
                  </div>
                  <div className="rounded-xl border border-emerald-300/40 bg-emerald-50/40 p-4 text-emerald-700 dark:border-emerald-500/40 dark:bg-emerald-500/10 dark:text-emerald-300 sm:col-span-2">
                    AI control is active now. You can ask Operon to create ad
                    sets, optimize spend, pause underperformers, and scale
                    winners.
                  </div>
                </div>
              </div>
              ...
              <div className="flex min-h-168 flex-col rounded-2xl border border-border/70 bg-card/60 p-6">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Open the dedicated workspace to connect assets, inspect
                  campaign insights, generate ad set blueprints, and execute
                  pause/resume safely.
                </p>
                <div className="mt-6 space-y-3">
                  <div className="rounded-xl border border-border/70 bg-background/90 p-4 text-sm text-foreground">
                    Rich account + campaign explorer with direct Meta Graph
                    queries.
                  </div>
                  <div className="rounded-xl border border-border/70 bg-background/90 p-4 text-sm text-foreground">
                    AI-generated ad set strategy cards grounded in live campaign
                    metrics.
                  </div>
                  <div className="rounded-xl border border-border/70 bg-background/90 p-4 text-sm text-foreground">
            ...
            <div className="rounded-2xl border border-dashed border-border/70 bg-card/40 p-6 text-sm text-muted-foreground">
              Connect Facebook first to unlock the deep AI ad operations
              workspace.
            </div>
          )}
        </div>
